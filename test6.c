#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
#include <sys/types.h>
#include <sys/wait.h>
#include <sys/stat.h>
#include <errno.h>
#include <stdarg.h>
#include <signal.h>
#include <limits.h>
#include <fcntl.h>

// --- Colors (for the prompt) ---  (Keep these if you're using them)
#define CLR_DEFAULT "\x1b[0m"
#define CLR_RED_BOLD "\x1b[1;31m"
#define CLR_GREEN_BOLD "\x1b[1;32m"
#define CLR_YELLOW_BOLD "\x1b[1;33m"
#define CLR_BLUE_BOLD "\x1b[1;34m"
#define CLR_DEFAULT_BOLD "\x1b[1m"

// --- Constants ---
#ifndef PATH_MAX // In case it's not already defined
#define PATH_MAX 4096
#endif

// --- Token Types ---
typedef enum
{
	TOKEN_COMMAND,
	TOKEN_AND,
	TOKEN_OR,
	TOKEN_SEMICOLON,
	TOKEN_LPAREN,
	TOKEN_RPAREN,
	TOKEN_PIPE,
	TOKEN_INPUT,  // <
	TOKEN_OUTPUT, // >
	TOKEN_APPEND  // >>
} TokenType;

typedef struct
{
	TokenType type;
	char *value; // For commands/arguments, and filenames
} Token;

// --- AST Node Structures ---

typedef struct CommandNode
{
	char **args;			  // The command and its arguments (array of strings)
	char *input_file;		  // For < redirection (filename)
	char *output_file;		  // For > and >> redirection (filename)
	int append;				  // 1 for >>, 0 for > (or no redirection)
	struct CommandNode *next; // For pipes (linked list of commands)
} CommandNode;

typedef struct ASTNode
{
	enum
	{
		NODE_COMMAND, // Represents a single command
		NODE_AND,	  // Represents &&
		NODE_OR		  // Represents ||
	} type;
	union
	{
		CommandNode *command; // If type == NODE_COMMAND
		struct
		{
			struct ASTNode *left;
			struct ASTNode *right;
		} binary; // If type == NODE_AND or NODE_OR
	} data;
} ASTNode;

// --- Function Prototypes ---

// --- From your original files (likely already present, but include them here) ---
void printPrompt(int isAtty, char *user, char *hostname, char *path);
void shellLoop(int isAtty, char *argv[]);
void handle_input(char *input, int isAtty, char *argv[]);
int execute_command(char *commandPath, char **arguments);
int execute_pipe_command(char **commands, int num_commands);
int handle_redirection(char **args); // Add prototype
char *findPath(char *command);
int customCmd(char **tokens, int isAtty, char *input); // Make sure this is here
void initialize_environ(void);						   // Make sure you have this prototype
void safeExit(int exit_code);
char **split_line(char *line, const char *delimiters); // parse command prototype
char *tokens_to_string(Token *tokens, int num_tokens);

// --- String utility functions (prototypes for your custom functions) ---
char *_strdup(const char *s);
int _strcmp(const char *s1, const char *s2);
size_t _strcspn(const char *s, const char *reject);
char *_strchr(const char *s, int c);
char *_strcpy(char *dest, const char *src);
char *_strcat(char *dest, const char *src);
char *_strstr(const char *haystack, const char *needle);
int _strncmp(const char *s1, const char *s2, size_t n);
char *_strtok_r(char *str, const char *delim, char **saveptr);
void free_args(char **tokens); // renamed

// --- NEW Functions (for AST and Tokenization) ---
Token *tokenize(char *input, int *num_tokens);
ASTNode *parse_command_group(char *command_group);
ASTNode *parse_and_or(char *command_group);
ASTNode *parse_parentheses(char *command_group);
ASTNode *parse_single_command(char *command_group);
ASTNode *create_ast_node(int type);
void free_command_node(CommandNode *cmd);
void free_ast(ASTNode *node);
void free_tokens(Token *tokens, int num_tokens);
int execute_ast(ASTNode *node);
int execute_logical(char *line);	 // Add prototype
char **parse_command(char *command); // Add for consistency

// --- External Variables ---

extern char **environ;	  // Make sure you have this!
extern int g_exit_status; // Declare the global exit status

// --- Helper Functions ---

void initialize_environ(void)
{
	char **new_environ;
	int i = 0, size = 0;

	/* Calculate initial size of environ */
	while (environ[size] != NULL)
		size++;
	new_environ = malloc((size + 1) * sizeof(char *));
	if (!new_environ)
	{
		perror("malloc failed");
		exit(1); /*  Exit early on allocation failure. */
	}
	for (i = 0; environ[i] != NULL; i++)
	{
		new_environ[i] = strdup(environ[i]); /* make copy */
		if (!new_environ[i])
		{
			perror("strdup failed");
			while (--i >= 0) /* Frees any copies made */
				free(new_environ[i]);
			free(new_environ);
			exit(1); /* Exit on failure */
		}
	}
	new_environ[size] = NULL; /* array has to be NULL terminated */
	environ = new_environ;	  /* makes copy global */
}

char *_strdup(const char *s)
{
	if (s == NULL)
		return NULL;
	size_t len = strlen(s) + 1;
	char *new_str = malloc(len);
	if (new_str == NULL)
		return NULL;
	return memcpy(new_str, s, len);
}
int _strcmp(const char *s1, const char *s2)
{
	while (*s1 && (*s1 == *s2))
	{
		s1++;
		s2++;
	}
	return *(unsigned char *)s1 - *(unsigned char *)s2;
}
size_t _strcspn(const char *s, const char *reject)
{
	size_t count = 0;
	while (*s)
	{
		if (strchr(reject, *s))
		{
			return count;
		}
		else
		{
			s++;
			count++;
		}
	}
	return count;
}
char *_strchr(const char *s, int c)
{
	while (*s != '\0')
	{
		if (*s == (char)c)
		{
			return (char *)s; // Cast away const for the return
		}
		s++;
	}
	if (c == '\0')
	{
		return (char *)s; // Special case: check for null terminator
	}
	return NULL;
}

char *_strcpy(char *dest, const char *src)
{
	char *start = dest;

	while (*src)
	{
		*dest = *src;
		dest++;
		src++;
	}
	*dest = '\0';
	return (start);
}

char *_strcat(char *dest, const char *src)
{
	char *start = dest;

	while (*dest)
		dest++;

	while (*src)
	{
		*dest = *src;
		dest++;
		src++;
	}

	*dest = '\0';
	return (start);
}

char *_strstr(const char *haystack, const char *needle)
{
	if (!*needle) /*  Empty needle */
		return ((char *)haystack);

	while (*haystack)
	{
		const char *h = haystack;
		const char *n = needle;

		while (*h && *n && (*h == *n))
		{
			h++;
			n++;
		}

		if (!*n) /*  Needle found */
			return ((char *)haystack);

		haystack++;
	}

	return (NULL); /*  Needle not found */
}

int _strncmp(const char *s1, const char *s2, size_t n)
{
	if (n == 0)
	{
		return (0);
	}
	while (--n && *s1 && *s1 == *s2)
	{
		s1++;
		s2++;
	}
	return (*(unsigned char *)s1 - *(unsigned char *)s2);
}
/**
 * _strtok_r - A reentrant version of strtok.
 * @str: The string to be tokenized.  If NULL, continues from last token.
 * @delim: The delimiter characters.
 * @saveptr: A pointer to a char*, used internally to store the next token.
 *
 * Return: A pointer to the next token, or NULL if no more tokens are found.
 */
char *_strtok_r(char *str, const char *delim, char **saveptr)
{
	char *token_start;
	char *token_end;

	if (str != NULL)
	{
		// New string to tokenize
		token_start = str;
	}
	else
	{
		// Continue from last saved position
		if (*saveptr == NULL)
		{
			return NULL; // No previous state, no more tokens
		}
		token_start = *saveptr;
	}

	// Skip leading delimiters
	while (*token_start && strchr(delim, *token_start))
	{
		token_start++;
	}

	if (*token_start == '\0')
	{
		*saveptr = NULL; // No more tokens
		return NULL;
	}

	// Find the end of the token
	token_end = token_start;
	while (*token_end && !strchr(delim, *token_end))
	{
		token_end++;
	}

	if (*token_end != '\0')
	{
		*token_end = '\0';		  // Null-terminate the token
		*saveptr = token_end + 1; // Save the next position
	}
	else
	{
		*saveptr = NULL; // No more tokens after this one
	}

	return token_start;
}

/**
 * free_args - Frees a NULL-terminated array of strings (char**).
 * @args: The array of strings to free.
 */
void free_args(char **args)
{ // Renamed from free_tokens
	if (args == NULL)
		return;
	for (int i = 0; args[i] != NULL; i++)
	{
		free(args[i]);
	}
	free(args);
}

//-----------------END STRING FUNCS----------------------
//-----------------START BUILTINS------------------------

/**
 * execute_builtin - checks for built-in commands, runs if exists
 *
 * @tokens: tokenized user input
 *
 * Return: 1 if custom, 0 if not
 */
int customCmd(char **tokens, int isAtty, char *input)
{
	if (!tokens || !tokens[0])
		return (0); /* handles NULL */

	if (!_strcmp(tokens[0], "exit"))
	{					   /* built-in #1 "exit" */
		int exit_code = 0; /* handles no arg exit */

		if (tokens[1])					 /* checks if user entered 2nd arg */
			exit_code = atoi(tokens[1]); /* makes 2nd arg the code */

		if (!isAtty)
			free(input);	 /* getline alloc'd so must be freed */
		free_args(tokens);	 /* frees tokens array */
		safeExit(exit_code); /* exits with appropriate code */
		return (1);
	}
	else if (!_strcmp(tokens[0], "env"))
	{ /* built-in #2 "env" */
		char **env = environ;

		while (*env != NULL)
		{
			printf("%s\n", *env);
			env++;
		}
		g_exit_status = 0;
		return (1);
	}
	else if (!_strcmp(tokens[0], "cd"))
	{
		char *dir = tokens[1];
		char *home_dir = getenv("HOME");

		if (dir == NULL || _strcmp(dir, "") == 0)
		{
			if (home_dir == NULL)
			{
				fprintf(stderr, "hsh: cd: HOME not set\n");
				g_exit_status = 1;
				return (1);
			}
			dir = home_dir;
		}
		else if (_strcmp(dir, "-") == 0)
		{
			char *prev_dir = getenv("OLDPWD");
			if (prev_dir == NULL)
			{
				fprintf(stderr, "hsh: cd: OLDPWD not set\n");
				g_exit_status = 1;
				return (1);
			}
			dir = prev_dir;
			printf("%s\n", dir); // Print the directory we're changing to
		}

		char cwd[1024]; // Buffer for current working directory
		if (getcwd(cwd, sizeof(cwd)) == NULL)
		{
			perror("hsh: getcwd failed");
			g_exit_status = 1;
			return (1);
		}

		if (chdir(dir) != 0)
		{
			perror("hsh: cd failed");
			g_exit_status = 1;
			return 1;
		}

		if (setenv("OLDPWD", cwd, 1) != 0) // Update OLDPWD *before* PWD
		{
			perror("hsh: setenv OLDPWD failed");
			g_exit_status = 1;
			return (1);
		}

		if (getcwd(cwd, sizeof(cwd)) == NULL)
		{
			perror("hsh: getcwd failed");
			g_exit_status = 1;
			return (1);
		}
		if (setenv("PWD", cwd, 1) != 0) // Update PWD
		{
			perror("hsh: setenv PWD failed");
			g_exit_status = 1;
			return (1);
		}

		g_exit_status = 0;
		return (1);
	}
	else if (_strcmp(tokens[0], "setenv") == 0)
	{
		if (tokens[1] == NULL || tokens[2] == NULL || tokens[3] != NULL)
		{
			fprintf(stderr, "hsh: setenv: Usage: setenv VARIABLE VALUE\n");
			g_exit_status = 1;
			return (1);
		}
		if (setenv(tokens[1], tokens[2], 1) != 0)
		{
			perror("hsh: setenv failed");
			g_exit_status = 1;
			return (1);
		}
		g_exit_status = 0;
		return (1);
	}
	else if (_strcmp(tokens[0], "unsetenv") == 0)
	{
		if (tokens[1] == NULL || tokens[2] != NULL)
		{
			fprintf(stderr, "hsh: unsetenv: Usage: unsetenv VARIABLE\n");
			g_exit_status = 1;
			return (1);
		}
		if (unsetenv(tokens[1]) != 0)
		{
			perror("hsh: unsetenv failed");
			g_exit_status = 1;
			return (1);
		}
		g_exit_status = 0;
		return (1);
	}
	return (0);
}

//-------------------END BUILTINS-----------------------
//-------------------FIND PATH--------------------------
/**
 * findPath - Locates the full path of a command.
 * @command: The command name.
 *
 * Return: Full path to the command if found and executable,
 *         otherwise returns a copy of the command.  The caller
 *         is responsible for freeing the returned string.
 */
char *findPath(char *command)
{
	char *path = getenv("PATH");
	char *path_copy;
	char *dir;
	char full_path[PATH_MAX];

	// If command contains a '/', it's an absolute or relative path
	if (_strchr(command, '/'))
	{
		if (access(command, X_OK) == 0)
		{
			return _strdup(command); // Return a copy, caller must free
		}
		else
		{
			return NULL; // Not found or not executable
		}
	}

	if (!path)
	{
		return NULL; // No PATH variable
	}

	path_copy = _strdup(path); // Duplicate PATH
	if (!path_copy)
	{
		perror("strdup");
		return NULL;
	}

	char *saveptr; // To be used by strtok

	dir = _strtok_r(path_copy, ":", &saveptr);
	while (dir != NULL)
	{
		snprintf(full_path, sizeof(full_path), "%s/%s", dir, command);
		if (access(full_path, X_OK) == 0)
		{
			free(path_copy);
			return _strdup(full_path); // Return a copy!
		}
		dir = _strtok_r(NULL, ":", &saveptr);
	}

	free(path_copy);
	return NULL; // Not found
}
//-------------------END FIND PATH------------------------
//-------------------START EXECUTER-----------------------
int handle_redirection(char **args)
{
	int i = 0;
	int fd;
	int flags = 0;	// To store flags for open
	int append = 0; // Flag to track >> operator

	while (args[i] != NULL)
	{
		if (_strcmp(args[i], ">") == 0)
		{
			flags = O_WRONLY | O_CREAT | O_TRUNC;
			append = 0; // Reset append flag for >
		}
		else if (_strcmp(args[i], ">>") == 0)
		{
			flags = O_WRONLY | O_CREAT | O_APPEND;
			append = 1;
		}
		else if (_strcmp(args[i], "<") == 0)
		{
			flags = O_RDONLY;
		}
		else
		{
			i++;
			continue; // Not a redirection operator, move to the next arg
		}

		if (args[i + 1] == NULL)
		{
			fprintf(stderr, "hsh: syntax error near unexpected token `newline'\n");
			return -1; // Syntax error: no file specified
		}

		// Handle the file
		if (_strcmp(args[i], "<") == 0)
		{
			fd = open(args[i + 1], flags);
		}
		else
		{
			fd = open(args[i + 1], flags, 0666); // Create with rw-rw-rw-
		}

		if (fd == -1)
		{
			perror("hsh: open");
			return -1; // File open failed
		}

		// Redirect
		if (_strcmp(args[i], "<") == 0)
		{
			if (dup2(fd, STDIN_FILENO) == -1)
			{
				perror("hsh: dup2");
				close(fd);
				return -1;
			}
		}
		else
		{ // ">" or ">>"
			if (dup2(fd, STDOUT_FILENO) == -1)
			{
				perror("hsh: dup2");
				close(fd);
				return -1;
			}
		}
		close(fd); // Close the file descriptor after dup2

		// Remove the redirection operator and filename from args
		free(args[i]);
		free(args[i + 1]);
		args[i] = NULL;
		args[i + 1] = NULL;

		// Shift the remaining args
		int j = i + 2;
		while (args[j] != NULL)
		{
			args[j - 2] = args[j];
			j++;
		}
		args[j - 2] = NULL; // Null terminate
	}
	return 0; // Success
}

int execute_command(char *commandPath, char **arguments)
{
	pid_t pid;
	int status;

	pid = fork();
	if (pid == -1)
	{
		perror("fork");
		return -1; // Indicate fork failure
	}
	else if (pid == 0)
	{
		// Child process
		// Handle Redirection
		if (handle_redirection(arguments) == -1)
		{
			exit(1); // Exit if redirection fails
		}

		// Attempt execvp first (search PATH)
		execve(commandPath, arguments, environ);

		// If execve returns, there was an error
		fprintf(stderr, "hsh: %s: command not found\n", arguments[0]);
		exit(127); // Command not found exit status
	}
	else
	{
		// Parent process
		waitpid(pid, &status, 0);
		if (WIFEXITED(status))
		{
			return WEXITSTATUS(status);
		}
		else if (WIFSIGNALED(status))
		{
			return 128 + WTERMSIG(status); // Consistent with bash signal handling
		}
		else
		{
			// Should not normally get here
			return -1; // Indicate an unexpected error
		}
	}
}

int execute_pipe_command(char **commands, int num_commands)
{
	if (num_commands <= 0)
	{
		return 0; /* Nothing to do */
	}

	if (num_commands == 1)
	{
		/* If there's only one command, we don't need pipes. */
		char **args = parse_command(commands[0]);
		if (args != NULL)
		{
			if (args[0] != NULL)
			{ /* check for empty command */
				char *full_path = findPath(args[0]);
				if (full_path != NULL)
				{
					g_exit_status = execute_command(full_path, args);
					free(full_path);
				}
				else
				{ /* command not found */
					fprintf(stderr, "%s: command not found\n", args[0]);
					g_exit_status = 127;
				}
			}
			free_args(args);
		}
		return g_exit_status;
	}

	/* --- Multiple commands, create pipes --- */

	int pipes[num_commands - 1][2]; /* Array of pipes */
	pid_t pids[num_commands];		/* Array to store child PIDs */

	/* Create all necessary pipes */
	for (int i = 0; i < num_commands - 1; i++)
	{
		if (pipe(pipes[i]) < 0)
		{
			perror("pipe");
			return 1; /* Handle pipe creation failure */
		}
	}

	/* Fork and execute each command */
	for (int i = 0; i < num_commands; i++)
	{
		pids[i] = fork();
		if (pids[i] < 0)
		{
			perror("fork");
			return 1; /* Handle fork failure */
		}
		else if (pids[i] == 0)
		{
			/* Child process */

			/* Redirect input (except for the first command) */
			if (i > 0)
			{
				if (dup2(pipes[i - 1][0], STDIN_FILENO) < 0)
				{
					perror("dup2 (stdin)");
					exit(EXIT_FAILURE);
				}
			}

			/* Redirect output (except for the last command) */
			if (i < num_commands - 1)
			{
				if (dup2(pipes[i][1], STDOUT_FILENO) < 0)
				{
					perror("dup2 (stdout)");
					exit(EXIT_FAILURE);
				}
			}

			/* Close all pipe ends in the child */
			for (int j = 0; j < num_commands - 1; j++)
			{
				close(pipes[j][0]);
				close(pipes[j][1]);
			}

			/* Parse the command */
			char **args = parse_command(commands[i]);
			if (args == NULL || args[0] == NULL)
			{
				fprintf(stderr, "Invalid command\n"); /* error message */
				exit(EXIT_FAILURE);
			}
			// Handle Redirection
			if (handle_redirection(args) == -1)
			{
				exit(1); // Exit if redirection fails
			}
			char *full_path = findPath(args[0]); /* find full path */
			if (full_path == NULL)
			{
				fprintf(stderr, "%s: command not found\n", args[0]);
				exit(127);
			}
			/* Execute the command */
			execve(full_path, args, environ);
			perror("execve"); /* If execve returns, it failed */
			exit(EXIT_FAILURE);
		}
	}

	/* Parent process: Close all pipe ends */
	for (int i = 0; i < num_commands - 1; i++)
	{
		close(pipes[i][0]);
		close(pipes[i][1]);
	}

	/* Wait for all child processes to finish */
	for (int i = 0; i < num_commands; i++)
	{
		waitpid(pids[i], NULL, 0);
	}
	return g_exit_status;
}
//-------------------END EXECUTER-----------------------
//-------------------START PARSER-----------------------
char **split_line(char *line, const char *delimiters)
{
	char **tokens = NULL;
	char *token;
	int bufsize = 64, position = 0;
	char *saveptr;

	tokens = malloc(
		bufsize * sizeof(char *));
	if (!tokens)
	{
		perror("hsh: allocation error");
		return NULL;
	}

	token = _strtok_r(line, delimiters, &saveptr);
	while (token != NULL)
	{
		tokens[position] = _strdup(token); // Duplicate the token!
		if (!tokens[position])
		{
			perror("hsh: allocation error");
			// Clean up previously allocated tokens
			while (--position >= 0)
			{
				free(tokens[position]);
			}
			free(tokens);
			return NULL;
		}
		position++;

		if (position >= bufsize)
		{
			bufsize += 64;
			tokens = realloc(tokens, bufsize * sizeof(char *));
			if (!tokens)
			{
				perror("hsh: allocation error");
				// Clean up previously allocated tokens (including the newly strdup'd one)
				while (--position >= 0)
				{
					free(tokens[position]);
				}
				free(tokens);
				return NULL;
			}
		}

		token = _strtok_r(NULL, delimiters, &saveptr);
	}
	tokens[position] = NULL;
	return tokens;
}
/**
 * parse_command - Parses a command string into an array of arguments.
 * @command: The command string.
 *
 * Return: A NULL-terminated array of strings (arguments) or NULL on error.
 */
char **parse_command(char *command)
{
	return split_line(command, " \t\r\n\a");
}

int split_command_line_on_pipe(char *input, char ***commands, int *num_commands)
{
	char *token;
	int count = 0;
	char *input_copy;

	/* Count the number of pipes */
	for (int i = 0; input[i] != '\0'; i++)
	{
		if (input[i] == '|')
		{
			count++;
		}
	}

	/* Allocate memory for commands array + NULL terminator */
	*commands = malloc(sizeof(char *) * (count + 2));
	if (*commands == NULL)
	{
		perror("malloc");
		return -1;
	}

	input_copy = _strdup(input); /* Duplicate the input string */
	if (input_copy == NULL)
	{
		perror("_strdup");
		free(*commands);
		return -1;
	}
	char *saveptr;
	token = _strtok_r(input_copy, "|", &saveptr); /* Tokenize based on pipe*/

	int i = 0;
	while (token != NULL)
	{
		(*commands)[i] = _strdup(token); /* Duplicate and store each token */
		if ((*commands)[i] == NULL)
		{
			perror("_strdup");
			/* Free previously allocated memory before returning */
			for (int j = 0; j < i; j++)
			{
				free((*commands)[j]);
			}
			free(*commands);
			free(input_copy);
			return -1;
		}
		i++;
		token = _strtok_r(NULL, "|", &saveptr); /* get next command */
	}

	(*commands)[i] = NULL; /* Null-terminate array */
	*num_commands = i;	   /* sets number of commands */
	free(input_copy);
	return 0; /* success */
}
// In logical_op.c
int handle_pipe_redirection(CommandNode *cmd)
{
	int input_fd = STDIN_FILENO;
	int output_fd = STDOUT_FILENO;
	int result = 0;

	if (cmd->input_file != NULL)
	{
		input_fd = open(cmd->input_file, O_RDONLY);
		if (input_fd == -1)
		{
			perror("open input file");
			return -1; // Or some other error code
		}
		if (dup2(input_fd, STDIN_FILENO) == -1)
		{
			perror("dup2 input");
			close(input_fd);
			return -1;
		}
		close(input_fd); // Close after dup2
	}

	if (cmd->output_file != NULL)
	{
		int flags = O_WRONLY | O_CREAT;
		flags |= (cmd->append) ? O_APPEND : O_TRUNC; // Correctly handle append
		output_fd = open(cmd->output_file, flags, 0666);
		if (output_fd == -1)
		{
			perror("open output file");
			return -1;
		}
		if (dup2(output_fd, STDOUT_FILENO) == -1)
		{
			perror("dup2 output");
			close(output_fd);
			return -1;
		}
		close(output_fd);
	}

	return result;
}
int pipe_command(CommandNode *cmd_list)
{
	int num_commands = 0;
	CommandNode *current = cmd_list;
	while (current != NULL)
	{
		num_commands++;
		current = current->next;
	}

	if (num_commands == 1)
	{
		// Execute single command with redirection, if any.
		if (handle_redirection(cmd_list->args) == -1)
			return 1; // Check for error
		char *full_path = findPath(cmd_list->args[0]);
		if (!full_path)
		{
			fprintf(stderr, "hsh: %s: command not found\n", cmd_list->args[0]);
			return 127;
		}
		int status = execute_command(full_path, cmd_list->args);
		free(full_path);
		return status;
	}
	// For multiple commands, create pipes.
	int pipes[num_commands - 1][2];
	pid_t pids[num_commands];

	for (int i = 0; i < num_commands - 1; i++)
	{
		if (pipe(pipes[i]) == -1)
		{
			perror("pipe");
			return 1; // Or another suitable error code
		}
	}
	// Iterate to run commands
	current = cmd_list;
	for (int i = 0; i < num_commands; i++)
	{
		pids[i] = fork();
		if (pids[i] == -1)
		{
			perror("fork");
			for (int j = 0; j < num_commands - 1; j++)
			{
				close(pipes[j][0]);
				close(pipes[j][1]);
			}
			return 1; // Handle fork failure
		}
		else if (pids[i] == 0)
		{
			// Child process

			// Redirect input (except for the first command)
			if (i > 0)
			{
				if (dup2(pipes[i - 1][0], STDIN_FILENO) < 0)
				{
					perror("dup2 stdin");
					exit(EXIT_FAILURE);
				}
			}
			// Redirect output (except for the last command)
			if (i < num_commands - 1)
			{
				if (dup2(pipes[i][1], STDOUT_FILENO) < 0)
				{
					perror("dup2 stdout");
					exit(EXIT_FAILURE);
				}
			}

			// Close all pipe ends in the child
			for (int j = 0; j < num_commands - 1; j++)
			{
				close(pipes[j][0]);
				close(pipes[j][1]);
			}

			// Handle input/output redirection for this command.
			if (handle_redirection(current->args) == -1)
			{
				exit(1); // Exit if redirection fails
			}

			// Execute
			execve(findPath(current->args[0]), current->args, environ);
			fprintf(stderr, "hsh: %s: command not found\n", current->args[0]);
			exit(127);
		}
		// Move to next command
		current = current->next;
	}
	// Parent process: Close all pipe ends
	for (int i = 0; i < num_commands - 1; i++)
	{
		close(pipes[i][0]);
		close(pipes[i][1]);
	}

	// Wait for all child processes to finish
	for (int i = 0; i < num_commands; i++)
	{
		waitpid(pids[i], NULL, 0);
	}
	return 0;
}

//-------------------END PARSER-----------------------
//-------------------START LOGICAL OPS----------------

// Forward declarations (needed because these functions are mutually recursive)
// --- Forward Declarations ---

ASTNode *parse_command_group(char *command_group);
ASTNode *parse_and_or(char *command_group);
ASTNode *parse_parentheses(char *command_group);
ASTNode *parse_single_command(char *command_group);

// --- Helper Functions ---

// Creates a new AST node.  Handles allocation and error checking.
ASTNode *create_ast_node(int type)
{
	ASTNode *node = malloc(sizeof(ASTNode));
	if (node == NULL)
	{
		perror("malloc");
		exit(EXIT_FAILURE); // Exit on allocation failure
	}
	node->type = type;
	return node;
}

// Frees a CommandNode (used by free_ast)
void free_command_node(CommandNode *cmd)
{
	if (cmd)
	{
		free_args(cmd->args); // Assuming you have a free_tokens function
		if (cmd->input_file)
			free(cmd->input_file);
		if (cmd->output_file)
			free(cmd->output_file);
		free(cmd);
	}
}

// Frees an AST node (and its children, recursively)
void free_ast(ASTNode *node)
{
	if (node == NULL)
	{
		return;
	}

	if (node->type == NODE_COMMAND)
	{
		free_command_node(node->data.command);
	}
	else
	{ // NODE_AND or NODE_OR
		free_ast(node->data.binary.left);
		free_ast(node->data.binary.right);
	}
	free(node);
}
// --- Parsing Functions ---

// Parses a single command (no logical operators, no parentheses)
ASTNode *parse_single_command(char *command_group)
{
	char **args = parse_command(command_group);
	if (args == NULL || args[0] == NULL)
	{
		free_args(args); // Handles empty
		return NULL;
	}

	ASTNode *node = create_ast_node(NODE_COMMAND);
	node->data.command = malloc(sizeof(CommandNode));
	if (!node->data.command)
	{
		perror("malloc in parse single command");
		free(node);
		free_args(args);
		exit(EXIT_FAILURE);
	}
	node->data.command->args = args;
	node->data.command->input_file = NULL;	// Initialize to NULL
	node->data.command->output_file = NULL; // Initialize to NULL
	node->data.command->append = 0;			// Initialize to 0
	node->data.command->next = NULL;		// Initialize to NULL
	return node;
}

// Handles parentheses:  ( command_group )
ASTNode *parse_parentheses(char *command_group)
{
	if (command_group[0] == '(')
	{
		int paren_count = 1;
		char *end = command_group + 1; // Start after the opening '('
		while (*end != '\0' && paren_count > 0)
		{
			if (*end == '(')
			{
				paren_count++;
			}
			else if (*end == ')')
			{
				paren_count--;
			}
			end++;
		}

		if (paren_count == 0)
		{
			// Null-terminate at the closing parenthesis to isolate the expression
			*(end - 1) = '\0';
			// Recursively parse the content *inside* the parentheses.
			return parse_command_group(command_group + 1);
		}
		else
		{
			fprintf(stderr, "hsh: syntax error: unmatched '('\n");
			g_exit_status = 2; // Set a syntax error status
			return NULL;	   // Indicate an error
		}
	}
	else
	{
		// No parentheses, parse as a single command
		return parse_single_command(command_group);
	}
}

// Parses sequences of && and ||
ASTNode *parse_and_or(char *command_group)
{
	ASTNode *left = parse_parentheses(command_group); // Highest precedence
	if (left == NULL)
	{
		return NULL; // Error during parsing
	}

	char *op = _strstr(command_group, "&&");
	if (op == NULL || (_strstr(command_group, "||") != NULL && op > _strstr(command_group, "||")))
	{
		op = _strstr(command_group, "||");
	}

	while (op != NULL)
	{
		ASTNode *node;
		if (_strncmp(op, "&&", 2) == 0)
		{
			node = create_ast_node(NODE_AND);
		}
		else
		{ // It must be "||" at this point
			node = create_ast_node(NODE_OR);
		}
		*op = '\0'; // Null-terminate at the operator.  This is KEY.
		node->data.binary.left = left;

		char *next_group = op + 2; // Skip "&&" or "||"
		while (*next_group == ' ' || *next_group == '\t')
		{
			next_group++; // Skip spaces after the operator
		}

		node->data.binary.right = parse_and_or(next_group); // Recursive call

		if (node->data.binary.right == NULL)
		{
			free_ast(node); // Clean up on error
			return NULL;	// Propagate error
		}
		left = node; // Chain together for multiple operators

		// Find next operator *correctly*
		op = _strstr(next_group, "&&"); // Start search AFTER current op
		if (op == NULL || (_strstr(next_group, "||") != NULL && op > _strstr(next_group, "||")))
		{
			op = _strstr(next_group, "||");
		}
	}

	return left; // Return the combined AST
}

// Top-level parsing function.  Splits by ';' and calls parse_and_or.
ASTNode *parse_command_group(char *command_group)
{
	return parse_and_or(command_group); // Just and/or
}

// --- Tokenization ---
void free_tokens(Token *tokens, int num_tokens)
{
	if (tokens == NULL)
		return;

	for (int i = 0; i < num_tokens; i++)
	{
		if (tokens[i].value != NULL)
		{
			free(tokens[i].value);
		}
	}
	free(tokens);
}
Token *tokenize(char *input, int *num_tokens)
{
	Token *tokens = malloc(sizeof(Token) * 64); // Initial allocation
	if (!tokens)
	{
		perror("malloc");
		exit(EXIT_FAILURE);
	}
	int token_count = 0;
	int capacity = 64;
	char *saveptr;
	char *token_str = _strtok_r(input, " \t\r\n", &saveptr); // Use a space tokenizer

	while (token_str != NULL)
	{

		// Handle logical operators and parentheses
		if (_strcmp(token_str, "&&") == 0)
		{
			tokens[token_count].type = TOKEN_AND;
			tokens[token_count].value = NULL;
		}
		else if (_strcmp(token_str, "||") == 0)
		{
			tokens[token_count].type = TOKEN_OR;
			tokens[token_count].value = NULL;
		}
		else if (_strcmp(token_str, ";") == 0)
		{
			tokens[token_count].type = TOKEN_SEMICOLON;
			tokens[token_count].value = NULL;
		}
		else if (_strcmp(token_str, "(") == 0)
		{
			tokens[token_count].type = TOKEN_LPAREN;
			tokens[token_count].value = NULL;
		}
		else if (_strcmp(token_str, ")") == 0)
		{
			tokens[token_count].type = TOKEN_RPAREN;
			tokens[token_count].value = NULL;
		}
		else if (_strcmp(token_str, "|") == 0)
		{
			tokens[token_count].type = TOKEN_PIPE;
			tokens[token_count].value = NULL;
		}
		else if (_strcmp(token_str, "<") == 0)
		{
			tokens[token_count].type = TOKEN_INPUT;
			tokens[token_count].value = NULL;
		}
		else if (_strcmp(token_str, ">") == 0)
		{
			tokens[token_count].type = TOKEN_OUTPUT;
			tokens[token_count].value = NULL;
		}
		else if (_strcmp(token_str, ">>") == 0)
		{
			tokens[token_count].type = TOKEN_APPEND;
			tokens[token_count].value = NULL;
		}

		else
		{ // Normal command or argument
			tokens[token_count].type = TOKEN_COMMAND;
			tokens[token_count].value = _strdup(token_str);
			if (!tokens[token_count].value)
			{
				perror("strdup");
				free_tokens(tokens, token_count); // Free previous
				exit(EXIT_FAILURE);
			}
		}

		token_count++;
		if (token_count >= capacity)
		{
			capacity *= 2;
			tokens = realloc(tokens, sizeof(Token) * capacity);
			if (!tokens)
			{
				perror("realloc");
				exit(EXIT_FAILURE);
			}
		}
		token_str = _strtok_r(NULL, " \t\r\n", &saveptr);
	}

	*num_tokens = token_count;
	return tokens;
}

// --- AST Execution ---

int execute_ast(ASTNode *node)
{
	if (node == NULL)
	{
		return 0; // Empty AST - nothing to do
	}

	if (node->type == NODE_COMMAND)
	{
		// Handle variable expansion *before* finding the path.
		for (int i = 0; node->data.command->args[i] != NULL; i++)
		{
			if (node->data.command->args[i][0] == '$')
			{
				char *var_name = node->data.command->args[i] + 1; // Skip the '$'
				char *var_value = getenv(var_name);
				if (var_value != NULL)
				{
					free(node->data.command->args[i]);
					node->data.command->args[i] = _strdup(var_value);
					if (node->data.command->args[i] == NULL)
					{
						perror("strdup");
						return 1; // Allocation error
					}
				}
			}
		}
		// Check for built-in commands *before* trying to execute as external
		if (customCmd(node->data.command->args, isatty(STDIN_FILENO), NULL) == 1)
		{
			return g_exit_status; // Built-in handled, return its status
		}

		char *full_path = findPath(node->data.command->args[0]);
		if (full_path == NULL)
		{
			fprintf(stderr, "hsh: %s: command not found\n", node->data.command->args[0]);
			return 127; // Command not found
		}
		int status = execute_command(full_path, node->data.command->args);
		free(full_path);
		return status;
	}
	else if (node->type == NODE_AND)
	{
		int left_status = execute_ast(node->data.binary.left);
		if (left_status == 0)
		{
			return execute_ast(node->data.binary.right);
		}
		return left_status; // Short-circuit
	}
	else if (node->type == NODE_OR)
	{
		int left_status = execute_ast(node->data.binary.left);
		if (left_status != 0)
		{
			return execute_ast(node->data.binary.right);
		}
		return left_status; // Short-circuit
	}
	return 0; // Should never reach here
}

// --- Top-Level Function (Splits by Semicolon) ---

int execute_logical(char *line)
{
	char *line_copy = _strdup(line); // Work on a copy
	if (line_copy == NULL)
	{
		perror("strdup");
		return 1;
	}

	int num_tokens = 0;
	ASTNode *ast = parse_command_group(line_copy);

	free(line_copy);

	if (!ast)
	{
		return g_exit_status;
	}
	g_exit_status = execute_ast(ast);
	free_ast(ast);

	return g_exit_status;
}
// Helper function to convert an array of tokens back into a string
char *tokens_to_string(Token *tokens, int num_tokens)
{
	if (num_tokens == 0)
	{
		return _strdup(""); // Handle empty token array
	}

	size_t total_length = 0;
	for (int i = 0; i < num_tokens; i++)
	{
		if (tokens[i].value != NULL)
		{
			total_length += strlen(tokens[i].value);
		}
		total_length++; // Space for space or null terminator
	}

	char *result = malloc(total_length + 1);
	if (result == NULL)
	{
		perror("malloc");
		exit(EXIT_FAILURE);
	}
	result[0] = '\0'; // Start with an empty string

	for (int i = 0; i < num_tokens; i++)
	{
		if (tokens[i].value != NULL)
		{
			strcat(result, tokens[i].value); // Concatenate token value
			if (i < num_tokens - 1)
			{
				strcat(result, " "); // Add a space between tokens
			}
		}
	}

	return result;
}
//---------------------END LOGICAL OPS-----------------
//-------------------START SHELL LOOP------------------
void handle_input(char *input, int isAtty, char *argv[])
{
	int num_tokens;
	Token *tokens = tokenize(input, &num_tokens);
	if (!tokens)
	{
		return; // Tokenization error (should have printed an error already)
	}

	// --- Split by Semicolons ---
	int start = 0;
	for (int i = 0; i <= num_tokens; i++)
	{
		// We check for end of tokens array OR a semicolon
		if (i == num_tokens || tokens[i].type == TOKEN_SEMICOLON)
		{
			// Process the tokens from 'start' up to (but not including) 'i'.
			// First, build a linked list of CommandNodes for potential pipes.

			CommandNode *command_list = NULL;
			CommandNode *last_command = NULL;
			int current_command_start = start;

			for (int j = start; j < i; j++)
			{
				if (tokens[j].type == TOKEN_PIPE)
				{
					// Found a pipe.  Everything from current_command_start to j-1
					// is a single command.
					tokens[j].type = TOKEN_COMMAND; // Treat as command for parsing
					char *command_str = tokens_to_string(&tokens[current_command_start], j - current_command_start);
					if (!command_str)
					{
						free_tokens(tokens, num_tokens);
						return;
					}
					char **args = parse_command(command_str); // Parse command into arguments
					if (!args)
					{
						free(command_str);
						free_tokens(tokens, num_tokens);
						return;
					}
					free(command_str);

					CommandNode *new_node = malloc(sizeof(CommandNode));
					if (!new_node)
					{
						perror("malloc");
						free_tokens(tokens, num_tokens);
						free_args(args);
						exit(EXIT_FAILURE);
					}
					new_node->args = args;
					new_node->input_file = NULL;
					new_node->output_file = NULL;
					new_node->append = 0;
					new_node->next = NULL;

					// Find redirection in this command segment
					for (int k = 0; args[k] != NULL; k++)
					{
						if (_strcmp(args[k], "<") == 0 && args[k + 1] != NULL)
						{
							new_node->input_file = _strdup(args[k + 1]);
							// Remove redirection tokens
							free(args[k]);
							free(args[k + 1]);
							args[k] = NULL;
							args[k + 1] = NULL;
							// Shift args
							int l = k + 2;
							while (args[l] != NULL)
							{
								args[l - 2] = args[l];
								l++;
							}
							args[l - 2] = NULL;
						}
						else if (_strcmp(args[k], ">") == 0 && args[k + 1] != NULL)
						{
							new_node->output_file = _strdup(args[k + 1]);
							new_node->append = 0;
							free(args[k]);
							free(args[k + 1]);
							args[k] = NULL;
							args[k + 1] = NULL;
							int l = k + 2;
							while (args[l] != NULL)
							{
								args[l - 2] = args[l];
								l++;
							}
							args[l - 2] = NULL;
						}
						else if (_strcmp(args[k], ">>") == 0 && args[k + 1] != NULL)
						{
							new_node->output_file = _strdup(args[k + 1]);
							new_node->append = 1; // Set append flag
							free(args[k]);
							free(args[k + 1]);
							args[k] = NULL;
							args[k + 1] = NULL;
							int l = k + 2;
							while (args[l] != NULL)
							{
								args[l - 2] = args[l];
								l++;
							}
							args[l - 2] = NULL;
						}
					}

					if (command_list == NULL)
					{
						command_list = new_node;
						last_command = new_node;
					}
					else
					{
						last_command->next = new_node;
						last_command = new_node;
					}
					current_command_start = j + 1; // Skip the pipe token
				}
			}

			// Process the last command (or the only command, if no pipes)
			if (current_command_start < i)
			{ // Check if there are remaining tokens
				char *command_str = tokens_to_string(&tokens[current_command_start], i - current_command_start);
				if (!command_str)
				{
					free_tokens(tokens, num_tokens);
					return;
				}
				char **args = parse_command(command_str); // Parse for arguments
				free(command_str);
				if (!args)
				{
					free_tokens(tokens, num_tokens);
					return;
				}
				CommandNode *new_node = malloc(sizeof(CommandNode));
				if (!new_node)
				{
					perror("malloc");
					exit(EXIT_FAILURE);
				}
				new_node->args = args;
				new_node->input_file = NULL;
				new_node->output_file = NULL;
				new_node->append = 0;
				new_node->next = NULL;

				// Find redirection in the last command segment
				for (int k = 0; args[k] != NULL; k++)
				{
					if (_strcmp(args[k], "<") == 0 && args[k + 1] != NULL)
					{
						new_node->input_file = _strdup(args[k + 1]);
						// Remove redirection tokens
						free(args[k]);
						free(args[k + 1]);
						args[k] = NULL;
						args[k + 1] = NULL;
						int l = k + 2;
						while (args[l] != NULL)
						{
							args[l - 2] = args[l];
							l++;
						}
						args[l - 2] = NULL;
					}
					else if (_strcmp(args[k], ">") == 0 && args[k + 1] != NULL)
					{
						new_node->output_file = _strdup(args[k + 1]);
						new_node->append = 0;
						free(args[k]);
						free(args[k + 1]);
						args[k] = NULL;
						args[k + 1] = NULL;
						int l = k + 2;
						while (args[l] != NULL)
						{
							args[l - 2] = args[l];
							l++;
						}
						args[l - 2] = NULL;
					}
					else if (_strcmp(args[k], ">>") == 0 && args[k + 1] != NULL)
					{
						new_node->output_file = _strdup(args[k + 1]);
						new_node->append = 1; // Set append flag
						free(args[k]);
						free(args[k + 1]);
						args[k] = NULL;
						args[k + 1] = NULL;
						int l = k + 2;
						while (args[l] != NULL)
						{
							args[l - 2] = args[l];
							l++;
						}
						args[l - 2] = NULL;
					}
				}

				if (command_list == NULL)
				{
					command_list = new_node;
				}
				else
				{
					last_command->next = new_node;
				}
			}
			// Execute the command list (handles pipes and redirection)
			if (command_list != NULL)
			{
				// Check if the command is a built-in *before* forking.
				if (command_list->args != NULL && customCmd(command_list->args, isAtty, input) == 1)
				{
					// It's a built-in, and it's been handled.
					g_exit_status = 0; // Builtin handled
				}
				else
				{
					// Now we check for logical operators, *before* piping
					char *command_group_str = tokens_to_string(&tokens[start], i - start);

					if (_strstr(command_group_str, "&&") || _strstr(command_group_str, "||"))
					{
						g_exit_status = execute_logical(command_group_str); // Execute the logical expression
					}
					else
					{
						g_exit_status = pipe_command(command_list);
					}
					free(command_group_str);
				}

				// Free the command list
				while (command_list != NULL)
				{
					CommandNode *next = command_list->next;
					free_args(command_list->args);
					if (command_list->input_file)
						free(command_list->input_file);
					if (command_list->output_file)
						free(command_list->output_file);
					free(command_list);
					command_list = next;
				}
			}

			start = i + 1; // Move past the semicolon
		}
	}

	free_tokens(tokens, num_tokens);
}

void shellLoop(int isAtty, char *argv[])
{
	size_t size;
	char *user, *hostname, path[PATH_MAX], *input;

	while (1)
	{
		// Initialize variables
		getcwd(path, sizeof(path));
		user = getenv("USER"); // Use getenv for USER
		hostname = getenv("HOSTNAME");
		if (!user)
			user = "user"; // Default val
		if (!hostname)
			hostname = "hostname";
		size = 0;
		input = NULL;

		printPrompt(isAtty, user, hostname, path);

		if (getline(&input, &size, stdin) == -1)
		{
			if (isAtty)
			{
				printf("\n%sCtrl-D Entered. %s\n", CLR_DEFAULT_BOLD, CLR_YELLOW_BOLD);
				printf("The %sGates Of Shell%s have closed. Goodbye.\n%s",
					   CLR_RED_BOLD, CLR_YELLOW_BOLD, CLR_DEFAULT);
			}
			free(input);			 // Free before safe exit
			safeExit(g_exit_status); // Exit with the last command's status
		}

		// Remove trailing newline
		input[_strcspn(input, "\n")] = 0;
		if (strlen(input) == 0) // Skip if empty
			continue;

		handle_input(input, isAtty, argv);

		free(input); // Free input
	}
}

/**
 * printPrompt - prints prompt in color ("[Go$H] | user@hostname:path$ ")
 *
 * @isAtty: is interactive mode
 * @user: environment variable for user's username
 * @hostname: environment variable for user's hostname or device name.
 * @path: current working directory
 */
void printPrompt(int isAtty, char *user, char *hostname, char *path)
{
	if (isAtty)
	{
		printf("%s[%sGo$H%s]%s | ", CLR_YELLOW_BOLD, CLR_RED_BOLD,
			   CLR_YELLOW_BOLD, CLR_DEFAULT);
		printf("%s%s@%s", CLR_GREEN_BOLD, user, hostname);
		printf("%s:%s%s", CLR_DEFAULT_BOLD, CLR_BLUE_BOLD, path);
		printf("%s$ ", CLR_DEFAULT);
		fflush(stdout); // Force print
	}
}

//---------------------END SHELL LOOP-----------------
int g_exit_status = 0; // Global variable to store the exit status
/**
 * safeExit - exits after freeing environ
 * @exit_code: exit code for exit()
 *
 * Return: void
 */
void safeExit(int exit_code)
{
	int i;

	if (environ)
	{
		for (i = 0; environ[i] != NULL; i++)
			free(environ[i]);
		free(environ);
		environ = NULL;
	}

	exit(exit_code);
}

int main(int argc, char *argv[])
{
	int isInteractive = isatty(STDIN_FILENO);

	(void)argc;

	if (isInteractive)
		printf("%sWelcome to the %sGates Of Shell%s. Type 'exit' to quit.\n\n",
			   CLR_YELLOW_BOLD, CLR_RED_BOLD, CLR_YELLOW_BOLD);

	initialize_environ();

	shellLoop(isInteractive, argv);
	if (isInteractive)
		printf("%s\nThe %sGates Of Shell%s have closed. Goodbye.\n%s",
			   CLR_YELLOW_BOLD, CLR_RED_BOLD, CLR_YELLOW_BOLD, CLR_DEFAULT);

	return (EXIT_SUCCESS); // Should never reach here in normal operation
}
