...
int execute_command_group(char *command_group)
{
	...
	// Recursive case: handle && and ||
	char *op = NULL;
	char *next_command_group = NULL;

	if (and_op != NULL && (or_op == NULL || and_op < or_op))
	{
		op = "&&";
		next_command_group = and_op + 2; // +2 to skip "&&"
		*and_op = '\0';					 // Null-terminate the first part
	}
	else if (or_op != NULL)
	{
		op = "||";
		next_command_group = or_op + 2;
		*or_op = '\0';
	}

	// Execute the first part
	int first_command_status = execute_command_group(command_group);

	if (_strcmp(op, "&&") == 0)
	{
		if (first_command_status == 0)
		{
			return execute_command_group(next_command_group); // Execute the next part
		}
		else
		{
			return first_command_status; // Skip the next part
		}
	}
	else
	{ // op == "||"
...
int execute_logical(char *line)
{
	...
	while (commands[i] != NULL)
	{
		...
		g_exit_status = execute_command_group(commands[i]);
		i++;
	}
...
void handle_input(char *input, int isAtty, char *argv[])
{
	int num_commands;
	char **commands = NULL;
	// Check for pipes *before* logical operators
	...
	else // Single command
	{
		...
		{ // Check for empty command
			...
			if (customCmd(tokens, isAtty, input) == 1)
			{
				...
				return;				 // Skip to next iteration if it's a built-in
			}
